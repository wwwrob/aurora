//! AURORA-SEAL-512: Secure-by-default authenticated encryption wrapper
//!
//! Provides authenticated encryption with associated data (AEAD) functionality
//! built on top of the AURORA-512 block cipher. This is the recommended
//! "secure-by-default" API for encrypting arbitrary-length data.

use crate::aurora512::Aurora512;
...
fn derive_subkey_512(master_key: [u8; KEY_SIZE], label: [u8; KEY_SIZE]) -> [u8; KEY_SIZE] {
    let aur = Aurora512::new(master_key);
    let mut block = label;
    aur.encrypt_block(&mut block);
    block
}

/// AURORA-SEAL-512 authenticated encryption wrapper
pub struct AuroraSeal512 {
    aurora_mac: Aurora512,
    aurora_enc: Aurora512,
}

impl AuroraSeal512 {
    /// Create a new AURORA-SEAL-512 instance with the given master key
    pub fn new(master_key: [u8; KEY_SIZE]) -> Self {
        // Derive MAC and encryption subkeys
        let mut l_mac = [0u8; KEY_SIZE];
        let mac_label = b"AURORA512:MAC";
        l_mac[..mac_label.len()].copy_from_slice(mac_label);
        // Rest is already zero

        let mut l_enc = [0u8; KEY_SIZE];
        let enc_label = b"AURORA512:ENC";
        l_enc[..enc_label.len()].copy_from_slice(enc_label);
        // Rest is already zero

        let k_mac = derive_subkey_512(master_key, l_mac);
        let k_enc = derive_subkey_512(master_key, l_enc);

        Self {
            aurora_mac: Aurora512::new(k_mac),
            aurora_enc: Aurora512::new(k_enc),
        }
    }
    ...
    fn stream_xor(&self, tag: &[u8; BLOCK_SIZE], input: &[u8]) -> Vec<u8> {
        ...
        for chunk in input.chunks(BLOCK_SIZE) {
            ...
            // XOR with input chunk - optimized: use extend_from_slice for full chunks
            if chunk.len() == BLOCK_SIZE {
                // Full block - use optimized XOR
...
mod tests {
    ...
    fn test_roundtrip_various_lengths() {
        ...
        let seal = AuroraSeal512::new(master_key);
